...
function getCurrentDateTime() { return dayjs().format('DD/MM/YYYY, HH:mm'); }
...
async function connectToMongoDB() {
   ...
   try {
      ...
      console.error("Erro ao connectar à MongoDB: ", error.message);
      throw error;
   }
}
...
connectToMongoDB().then(({ dbJRMFerias, dbRepairData, dbProdutosElectrex }) => {
   ...
   app.listen(port, '192.168.0.12', () => {
      ...
      console.log(`Servidor a correr em http://192.168.0.12:${port} at ${currentDateTime}`);
   });
}).catch((error) => {
